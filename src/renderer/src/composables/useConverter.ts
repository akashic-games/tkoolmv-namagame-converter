...
const DEFAULT_CONVERTER_MESSAGE = "ここに ツクールMVゲームの配布用ファイルをドラッグ＆ドロップしてください。";
...
interface State {
	...
	reset: () => void;
	jumpToGamePage: () => void;
	checkUsePluginConverter: (event: Event) => void;
}
...
export function useConverter() {
	...
	const onDrop = async (payload: DragEvent): Promise<void> => {
		...
		try {
			...
			if (state.isConverting || state.gameZip || !payload.dataTransfer?.files?.length || !payload.dataTransfer?.items?.length) return;
			...
			state.gameDistDirPath = await (window as any).tkoolmvApi.generateNamaGameDir(gameSrcDirPath, state.usePluginConverter);
			const ffmpegUrl = await (window as any).tkoolmvApi.getFfmpegUrl();
			await generateNamagameAudioAssets(gameSrcDirPath, state.gameDistDirPath, ffmpegUrl);
	...
	const checkUsePluginConverter = (_event: Event): void => {
		state.usePluginConverter = !state.usePluginConverter;
	};
