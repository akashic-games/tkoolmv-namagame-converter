...
export async function generateNamagameAudioAssets(gameSrcDirPath: string, gameDistDirPath: string, ffmpegBaseUrl: string): Promise<void> {
	try {
		...
		for (let i = 0; i * unit < audioData.length; i++) {
			await ffmpeg.load({
				coreURL: await FFmpegUtil.toBlobURL(`${ffmpegBaseUrl}/ffmpeg-core/ffmpeg-core.js`, "text/javascript"),
				wasmURL: await FFmpegUtil.toBlobURL(`${ffmpegBaseUrl}/ffmpeg-core/ffmpeg-core.wasm`, "application/wasm")
			});
